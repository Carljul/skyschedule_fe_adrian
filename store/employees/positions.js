...
export const actions = {
    async fetchEntry({ state, commit }) {
        ...
        try {
            ...
            const res = await this.$axios.get(`${app.getOrgUID}/positions`, {
    ...
    async fetchByUID({commit}, payload) {
        ...
        if(!payload) { return; }
        try {
            const res = await this.$axios.get(`${app.getOrgUID}/positions/${payload}`);
            commit('setState', {
                selected: {
                    ...res.data.response,
                    __title: res.data.response.position
                }
            });
            app.$nextTick(() => {
                app.findPageComponent('EmployeesPositionsEdit').$refs.modal_editentry.open=true;
            });
    ...
    async removeEntry({ dispatch }, payload) {
        const app = this._vm;
        try {
            await this.$axios.delete(`${app.getOrgUID}/positions/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async restoreEntry({dispatch}, payload) {
        const app = this._vm;
        try {
            await this.$axios.patch(`${app.getOrgUID}/positions/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async StoreUpdateEntry({ state, commit }) {
        const app = this._vm; 
        try {
            const res = await this.$axios.put(`${app.getOrgUID}/positions/${state.state.selected.uid}`, state.state.selected);
    ...
    async StoreInsertEntry({ state, dispatch, commit }) {
        const app = this._vm; 
        try {
            await this.$axios.post(`${app.getOrgUID}/positions`, state.state.inputs);
            app.notify({ title: 'Saved!', html: 'New Position has been saved.' });
            dispatch('fetchEntry');
            commit('resetInputs');
        } catch($e) {
            throw $e;
