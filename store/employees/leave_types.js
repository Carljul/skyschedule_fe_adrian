...
export const actions = {
    async fetchEntry({ state, commit }) {
        ...
        try {
            ...
            const res = await this.$axios.get(`${app.getOrgUID}/leave_types`, {
    ...
    async fetchByUID({commit}, payload) {
        ...
        if(!payload) { return; }
        try {
            const res = await this.$axios.get(`${app.getOrgUID}/leave_types/${payload}`);
            commit('setState', {
                selected: {
                    ...res.data.response,
            ...
            app.$nextTick(() => {
                app.findPageComponent('EmployeesLeaveTypesEdit').$refs.modal_editentry.open=true;
            });
    ...
    async removeEntry({ dispatch }, payload) {
        const app = this._vm;
        try {
            await this.$axios.delete(`${app.getOrgUID}/leave_types/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async restoreEntry({dispatch}, payload) {
        const app = this._vm;
        try {
            await this.$axios.patch(`${app.getOrgUID}/leave_types/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async StoreUpdateEntry({ state, commit }) {
        const app = this._vm;
        try {
            const res = await this.$axios.put(`${app.getOrgUID}/leave_types/${state.state.selected.uid}`, state.state.selected);
    ...
    async StoreInsertEntry({ state, commit, dispatch }) {
        const app = this._vm;
        try {
            await this.$axios.post(`${app.getOrgUID}/leave_types`, state.state.inputs);
            dispatch('fetchEntry');
            commit('resetInputs');
            app.notify({ title: 'Saved!', html: 'New Leave Type has been saved.' });
        } catch($e) {
            throw $e;
        }
    }
