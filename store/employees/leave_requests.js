...
export const actions = {
    async fetchEntry({ state, commit }) {
        ...
        try {
            ...
            console.log(state.state.entry.filter)
            const res = await this.$axios.get(`${app.getOrgUID}/leaves`, {
    ...
    async fetchByUID({commit}, payload) {
        ...
        if(!payload) { return; }
        try {
            const res = await this.$axios.get(`${app.getOrgUID}/leaves/${payload}`);
            commit('setState', {
                selected: {
                    ...res.data.response,
            ...
            app.$nextTick(() => {
                app.findPageComponent('EmployeesLeavesEdit').$refs.modal_editentry.open=true;
            });
    ...
    async removeEntry({ dispatch }, payload) {
        const app = this._vm;
        try {
            await this.$axios.delete(`${app.getOrgUID}/leaves/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async restoreEntry({dispatch}, payload) {
        const app = this._vm;
        try {
            await this.$axios.patch(`${app.getOrgUID}/leaves/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async StoreUpdateEntry({ state, commit }) {
        const app = this._vm;
        try {
            const res = await this.$axios.put(`${app.getOrgUID}/leaves/${state.state.selected.uid}`, state.state.selected);
    ...
    async StoreInsertEntry({ state, commit, dispatch }) {
        const app = this._vm;
        try {
            await this.$axios.post(`${app.getOrgUID}/leaves`, state.state.inputs);
            dispatch('fetchEntry');
            commit('resetInputs');
    ...
    async saveEntry({ commit }, payload) {
        const app = this._vm;
        try {
            const res = await this.$axios.put(`/${app.getOrgUID}/leaves/${payload.uid}`, payload);
            commit('updateEntryDataResponse', res.data.response);
        } catch($e) {
            throw $e;
        }
