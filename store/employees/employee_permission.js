...
export const actions = {
    async fetchEntry({ state, commit }) {
        ...
        try {
            ...
            const res = await this.$axios.get(`${app.getOrgUID}/permission-access/employees`, {
                params: {
                    ...state.state.entry.filter,
                    user_uid: state.state.employee.user.uid
                }
            });
    ...
    async removeEntry({ dispatch }, payload) {
        const app = this._vm;
        try {
            await this.$axios.delete(`${app.getOrgUID}/permission-access/employees/${payload.uid}`);
            dispatch('fetchEntry');
        } catch($e) {
            app.errorHandle($e);
        }
    ...
    async StoreEntry({ state, dispatch }) {
        const app = this._vm; 
        try {
            await this.$axios.post(`${app.getOrgUID}/permission-access/employees`, {
                ...state.state.inputs,
                user_uid: state.state.employee.user.uid
            });
    ...
    async permissionUpdate({ commit }, payload) {
        const app = this._vm; 
        try {
            const res = await this.$axios.patch(`${app.getOrgUID}/permission-access/employees/${payload.uid}`, {
