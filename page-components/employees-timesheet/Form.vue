<template>
    ...
    @close="() => {
        resetInputs();
        $router.push('/employees/timesheets');
    }"
    >
        ...
        <template slot="content">
            ...
            <ValidationObserver ref="adjustment" v-slot="{ handleSubmit }">
                <form @submit.prevent="handleSubmit(updateAdjustment)">
                    ...
                    <ValidationProvider ref="adjustment_shift_start" name="Shift Start" v-slot="{ validate, errors }" rules="required">
                        <div class="form-group-wrap mb-2">
                            <label class="mb-2 block text-xs font-medium --text-dark">Shift Start</label>
                            <client-only>
                                <t-datepicker mode="time" 
                                :value="state.inputs.shift_start"
                                @input="e => { 
                                    setState({ handle: 'state.inputs', key: 'shift_start', value: e });
                                }"
                                v-vchecker="errors[0]"
                                ></t-datepicker>
                            </client-only>
                            ...
                            :value="state.inputs.shift_start"
                            @input="e => { 
                                setState({ handle: 'state.inputs', key: 'shift_start', value: e.target.value });
                                validate(e); 
                            }"
                            >
                    ...
                    <ValidationProvider ref="adjustment_shift_end" name="Shift End" v-slot="{ validate, errors }" rules="required">
                        <div class="form-group-wrap mb-2">
                            <label class="mb-2 block text-xs font-medium --text-dark">Shift End</label>
                            <client-only>
                                <t-datepicker mode="time" 
                                :value="state.inputs.shift_end"
                                @input="e => { 
                                    setState({ handle: 'state.inputs', key: 'shift_end', value: e });
                                }"
                                v-vchecker="errors[0]"
                                ></t-datepicker>
                            </client-only>
                            ...
                            :value="state.inputs.shift_end"
                            @input="e => { 
                                setState({ handle: 'state.inputs', key: 'shift_end', value: e.target.value });
                                validate(e); 
                            }"
                            >
                            <v-msg :error="errors[0]" />
...
export default {
    name: 'EmoloyeeTimeAdjustmentForm',
    components: {
        AsyncButton,
        Modal,
        ValidationProvider,
