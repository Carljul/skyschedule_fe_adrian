<template>
    <div class="chat-media-files-container fixed z-50 bg-opacity-40 w-full h-full bg-black top-0 left-0 flex">
        <div class="chat-media-files w-96 bg-white m-auto rounded-lg relative">
            <div @click="opeAddMemberFunction" class="chat-files-close-btn absolute top-1 right-1 cursor-pointer">
            ...
            <div class="p-5">
                <div class="modal-details">
                    <h3 class="profile-name text-xl">Add Member</h3>
                </div>
                <div>
                    <form ref="add_group_member_form" @submit.prevent="addGroupMember">
                        <div class="mt-2 mb-2">
                            <ValidationProvider ref="newentry_user_uid" name="Department" v-slot="{ validate, errors }" rules="required">
                                <div class="form-group-wrap mb-3">
                                    ...
                                    <client-only>
                                        ...
                                        multiple="true"
                                        v-vchecker="errors[0]"
                                        @input="e => setState({ inputs: { ...state.inputs, users: e, group_uid: $route.params.uid } })"
                                        ></t-select-dynamic>
                                        <input type="hidden" 
  ...
  components: {
    ...
    methods: {
        ...
        async addGroupMember(){
            ...
            await this.AddGroupMember(this.$route.params.uid);
            this.opeAddMemberFunction();
