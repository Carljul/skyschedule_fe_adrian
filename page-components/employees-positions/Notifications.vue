    ...
    >
        <template slot="header">
            <h4 class="--text-heading text-lg font-semibold">
                <span class="block">{{state.data.position}}</span>
...
export default {
    name: 'PositionNotificationsModal',
    components: {
        Modal,
        SwitchToggle
    },
    computed: {
        ...mapState({
            state: state => state.employees.position_notification.state
        }),
        notificationKeys() {
            return {
    ...
    methods: {
        ...mapMutations('employees/position_notification', [
            'resetInputs',
            'setState'
        ]),
        ...mapActions('employees/position_notification', [
            'entryPostOrUpdate'
        ])
    }
}
