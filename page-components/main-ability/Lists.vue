            ...
            <tbody v-if="!state.entry.loading" class="bg-white divide-y divide-gray-200">
                <tr v-for="(entry, entryindex) in state.entry.data" :key="`department-entry-${genKey(entry)}`" class="transition-all hover:bg-gray-100">
                    ...
                    <template v-if="!entry.deleted_at">
                        ...
                        <nuxt-link  :to="`/ability?id=${entry.id}`" 
                        ...
                        <a href="#" @click.prevent="() => {
                            $refs.alertconfirm.$alert({
                                ...
                                execute: async function() {
                                    ...
                                    notify({ title: 'Success!', html: `Ability ${appdefaults.trashConfirm.success}` });
                                }
                            });
                    ...
                    <template v-else>
                        ...
                        @click.prevent="() => {
                            $refs.alertconfirm.$alert({
                                ...
                                execute: async function() {
                                    ...
                                    notify({ title: 'Success!', html: `Ability ${appdefaults.restoreConfirm.success}` });
                                }
                            });
                        }"
                        ...
                        <a href="#" @click.prevent="() => {
                            $refs.alertconfirm.$alert({
                                ...
                                execute: async function() {
                                    await removeEntry(entry);
                                    notify({ title: 'Success!', html: `Ability ${appdefaults.removeConfirm.success}` });
                                }
                            });
