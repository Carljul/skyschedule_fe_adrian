<template>
    <div class="flex sm:items-center justify-between py-3 border-b border-gray-200">
        <div class="relative py-2 pl-0 pr-3 cursor-pointer rounded flex items-center space-x-4 transition ease-in-out hover:bg-gray-200">
            ...
            <div @click="openMediaFilesFunctionDirect(state.user_selected)" class="relative">
                <span class="absolute text-green-500 right-0 bottom-0">
                    <svg width="20" height="20">
                        <circle cx="8" cy="8" r="8" fill="currentColor"></circle>
                    </svg>
                </span>
                <img v-if="header_data != null" :src="state.user_selected.user.image.url" alt="" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">
                <img v-if="header_data == null" src="https://images.unsplash.com/photo-1549078642-b2ba4bda0cdb?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=3&w=144&h=144" alt="" class="w-10 sm:w-16 h-10 sm:h-16 rounded-full">                
            </div>
            <div @click="openMediaFilesFunctionDirect(state.user_selected)" class="flex flex-col leading-tight">
                <div class="text-lg mt-1 flex items-center">                    
                <span v-if="header_data != null" class="text-gray-700 mr-3">{{ state.user_selected.user.full_name }}</span>
                </div>
                <span v-if="header_data != null" class="text-sm text-gray-600">{{ state.user_selected.user.position.position }}</span>
            </div>
        </div>
...
export default {
    ...
    props: ['openMediaFilesFunctionDirect', 'openMediaFilesFunction'],
    data(){
        return {
            header_data:null,
        }
    ...
    async mounted() {
        if(this.$route.params.uid) {
            await this.fetchSingleUser(this.$route.params.uid);
            console.log("HEADER STATE SHUSHUssssssssss", this.state);
            this.header_data = this.state.user_selected.user;
