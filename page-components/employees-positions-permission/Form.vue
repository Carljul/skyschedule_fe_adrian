            ...
            <template slot="content">
                ...
                <ValidationObserver ref="newentry" v-slot="{ handleSubmit }">
                    <form @submit.prevent="handleSubmit(addEntry)">
                        ...
                        <ValidationProvider ref="newentry_permission_uid" name="Permission" v-slot="{ validate, errors }" rules="required">
                            <div class="form-group-wrap mb-3">
                                <label class="mb-2 block text-xs font-medium --text-dark">Permission</label>
                                <client-only>
                                    <t-select-dynamic 
                                    :endpoint="`/permissions`" 
                                    datakeylabel="permission" 
                                    datakeyvalue="uid"
                                    searchplaceholder="Type to Search Permission"
                                    placeholder="Choose Permission"
                                    class="py-1.5"
                                    v-vchecker="errors[0]"
                                    @input="e => setState({ handle: 'state.inputs', key: 'permission_uid', value: e })"
                                    ></t-select-dynamic>
                                    <input type="hidden" 
                                    :value="state.inputs.permission_uid" 
                                    @input="e => {
                                        setState({ handle: 'state.inputs', key: 'permission_uid', value: e.target.value }); 
                                        validate(e); 
                                    }">
                                </client-only>
                        ...
                        <div class="form-group-wrap mb-3">
                            ...
                            :value="state.inputs.permission_value"
                            @input="e => {
                                setState({ handle: 'state.inputs', key: 'permission_value', value: e }); 
                            }"
                            >
                                <template slot="prepend">
                                    <span class="text-sm --text-dark mr-1" v-if="state.inputs.permission_value">Enabled</span>
                                    <span class="text-sm --text-dark mr-1" v-else>Disabled</span>
                                </template>
                            </SwitchToggle>
                        </div>
...
export default {
    name: 'EmployeesPositionPermissionForm',
    components: {
        AsyncButton,
        Modal,
        TSelectDynamic,
    ...
    methods: {
        ...mapMutations('employees/position_permission', [
            'setState',
            'resetInputs'
        ]),
