<template>
    <div class="w-full sm:w-80 max-w-full">
        <form class="flex items-center" @submit.prevent="fetchUsers" novalidate>
            ...
            <div class="relative w-full">
                ...
                :value="state.users.filter.query"
                @input="e => setState({ users: { ...state.users, filter: { ...state.users.filter, query: e.target.value } } })"
                >
            </div>
        </form>
        ...
        <client-only>
            <t-select-status 
            :value="{with_trashed:state.users.filter.with_trashed,only_trashed:state.users.filter.only_trashed}"
            class="mt-3" 
            @input="e => {
                setState({ users: { ...state.users, filter: { ...state.users.filter, ...e } } });
                fetchUsers();
            }"></t-select-status>
        </client-only>
