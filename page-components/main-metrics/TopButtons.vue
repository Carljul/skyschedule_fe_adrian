<template>
    <div class="mb-5 w-full items-start block sm:flex">
        ...
        <div class="ml-0 lg:ml-3 inline-flex items-center">
            ...
            <button type="button" 
            @click="() => { $scrollToView('.reminders-tab') }"
            :class="`p-2 ${btnclassnormal} ml-2`"
            >Reminders</button>
        </div>
...
export default {
    ...
    methods: {
        ...
        openNewMetrics() {
            this.resetMetricsInput();
            this.findPageComponent('MainMetricsFormModal').$refs.postentrymodal.open=true;
        }
    }
}
</script>