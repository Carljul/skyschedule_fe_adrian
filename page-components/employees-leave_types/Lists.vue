                    ...
                    <div class="overflow-hidden border-b border-gray-200 shadow" :class="{ 'shadow opacity-50 pointer-events-none': state.entry.loading }">
                        ...
                        <table class="w-full overflow-x-scroll divide-y divide-gray-200">
                            ...
                            <tbody v-if="!state.entry.loading" class="bg-white divide-y divide-gray-200">
                                <tr v-for="(entry, entryindex) in state.entry.data" :key="`leave_type-entry-${genKey(entry)}`" class="transition-all hover:bg-gray-100">
                                    <td class="px-6 py-4">
                                        <span class="--text-dark text-sm font-semibold">{{ entry.title }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="--text-dark text-sm font-semibold">{{ entry.description }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium text-right flex items-center justify-end">
                                        ...
                                        <template v-if="!entry.deleted_at">
                                            <nuxt-link
                                            :to="`/employees/leave_types?uid=${entry.uid}`"
                                            ...
                                            <a href="#" @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        ...
                                                        notify({ title: 'Success!', html: `Leave Type ${appdefaults.trashConfirm.success}` });
                                                    }
                                                });
                                        ...
                                        <template v-else>
                                            ...
                                            @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        ...
                                                        notify({ title: 'Success!', html: `Leave Type ${appdefaults.restoreConfirm.success}` });
                                                    }
                                                });
                                            }"
                                            ...
                                            <a href="#" @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        await removeEntry(entry);
                                                        notify({ title: 'Success!', html: `Leave Type ${appdefaults.removeConfirm.success}` });
                                                    }
                                                });
