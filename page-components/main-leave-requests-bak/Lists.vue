                        ...
                        <table class="w-full overflow-x-scroll divide-y divide-gray-200">
                            ...
                            <tbody v-if="!state.entry.loading" class="bg-white divide-y divide-gray-200">
                                <tr v-for="(entry, entryindex) in state.entry.data" :key="`department-entry-${genKey(entry)}`" class="transition-all hover:bg-gray-100">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="--text-dark text-sm font-semibold">{{ entry.start_date }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span class="--text-dark text-sm font-semibold">{{ entry.status }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm font-medium text-right flex items-center justify-end">

                                        <template v-if="!entry.deleted_at">
                                            <nuxt-link
                                            :to="`/employees/leave-requests?uid=${entry.uid}`"
                                            ...
                                            <a href="#" @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        ...
                                                        notify({ title: 'Success!', html: `Department ${appdefaults.trashConfirm.success}` });
                                                    }
                                                });
                                        ...
                                        <template v-else>
                                            ...
                                            @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        ...
                                                        notify({ title: 'Success!', html: `Department ${appdefaults.restoreConfirm.success}` });
                                                    }
                                                });
                                            }"
                                            ...
                                            <a href="#" @click.prevent="() => {
                                                $refs.alertconfirm.$alert({
                                                    ...
                                                    execute: async function() {
                                                        await removeEntry(entry);
                                                        notify({ title: 'Success!', html: `Department ${appdefaults.removeConfirm.success}` });
                                                    }
                                                });
