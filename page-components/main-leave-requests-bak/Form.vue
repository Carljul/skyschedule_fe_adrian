    ...
    >
        <template slot="header">
            <h4 class="--text-heading text-lg font-semibold">
                <span v-if="state.inputs.uid">Edit Overtime</span>
                <span v-else>Request Overtime</span>
            </h4>
        </template>
                ...
                <form @submit.prevent="handleSubmit(savePostEntry)">
                    ...
                    <div class="form-group-wrap">
                        <async-button ref="adjustment_btn" type="submit" :class="`p-3 ${btnclassnormal} mt-3`">
                            <span v-if="state.inputs.uid">Save Overtime</span>
...
export default {
    ...
    computed: {
        ...mapState({
            state: state => state.main.overtime.state
        }),
    },
    methods: {
        ...mapMutations('main/overtime', [
            'setState',
            'resetInputs'
        ]),
        ...mapActions('main/overtime', [
            'saveEntry'
        ]),
        async savePostEntry() {
            try {
