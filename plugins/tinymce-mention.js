...
if(process.client) {
    ...
    AutoComplete.prototype = {
        ...
        renderInput: function () {
            var rawHtml =  '<span class="autocomplete-module-tinymce">' +
        ...
        rteClicked: function (e) {
            ...
            if (this.hasFocus && $target.parent().attr('class') !== 'autocomplete-searchtext') {
                this.cleanUp(true);
            }
        },
        ...
        renderDropdown: function () {
            return `<ul class="rte-autocomplete dropdown-menu">
                <li class="text-center"><div class="m-loader-ring primary"></div></li>
            </ul>`;
        },
    
        render: function (item) {
            return '<li>' +
                        '<a href="javascript:;"><span>@' + `${item[this.options.queryBy].toLowerCase()}${item[this.options.queryBy2].toLowerCase()}` + '</span></a>' +
                    '</li>';
        },
        ...
        select: function (item) {
            this.editor.focus();
            var selection = this.editor.dom.select('span.autocomplete-module-tinymce')[0];
        ...
        cleanUp: function (rollback) {
            try {
                ...
                if (rollback) {
                    var text = this.query,
                        $selection = $(this.editor.dom.select('span.autocomplete-module-tinymce')),
        ...
        offset: function () {
            var rtePosition = $(this.editor.getContainer()).offset(),
                ...
                nodePosition = $(this.editor.dom.select('span.autocomplete-module-tinymce')).position();
        ...
        offsetInline: function () {
            var nodePosition = $(this.editor.dom.select('span.autocomplete-module-tinymce')).offset();
    ...
    tinymce.PluginManager.add('mention', function(ed, url) {
        ...
        return {
            getMetadata: function () {
              return  {
                name: "Mention Plugin",
                url: "https://www.facebook.com/jundell.hentai/"
              };
            }
          };
    });
