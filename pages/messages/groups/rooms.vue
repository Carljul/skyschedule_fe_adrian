        ...
        <div class="chatrooms-group-message py-2">
            ...
            <div class="flex-1 h-full px-2 chatrooms">
                ...
                <p @click="nuxtClosingLink">
                    <nuxt-link v-for="(entry, entryindex) in state.entry.data" :key="`department-entry-${genKey(entry)}`"  :to="`?uid=${entry.uid}`"  class="entry cursor-pointer transform hover:scale-105 duration-300 transition-transform bg-white mb-4 rounded p-4 flex shadow-md border-right-green">
